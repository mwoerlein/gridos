...
$(BOOTDIR)/$(MASCHINE)_Kernel-JIT.bin: $(BOOTDIR)/$(MASCHINE)_bootstrap_entry.o $(BOOTDIR)/$(MASCHINE)_bootstrap.o $(LIBDIR)/KernelJIT.a $(LIBDIR)/$(MASCHINE).a $(LIBDIR)/memory.a $(LIBDIR)/multiboot2.a 
	echo "creating $@"
	ld -e bootstrap_start -Ttext 0x60000 -s --oformat binary -m elf_i386 -o $@ $< $(BOOTDIR)/$(MASCHINE)_bootstrap.o $(LDHEAD) $(LIBDIR)/$(MASCHINE).a $(LIBDIR)/KernelJIT.a $(LIBDIR)/memory.a $(LIBDIR)/multiboot2.a $(LDTAIL)
